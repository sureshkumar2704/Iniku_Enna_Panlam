...
exports.handler = async function (event) {
  ...
  const id = qs.id || (qs.ids && qs.ids.split(',')[0]);

  try {
    if (method === 'GET') {
      if (id) {
        const q = { id: `eq.${id}` };
        const r = await supabaseRequest('tasks', 'GET', q);
        return { statusCode: r.status, body: JSON.stringify(r.data), headers: cors() };
      }
      const r = await supabaseRequest('tasks', 'GET');
      return { statusCode: r.status, body: JSON.stringify(r.data), headers: cors() };
    }

    if (method === 'POST') {
      const body = JSON.parse(event.body || '{}');
      const r = await supabaseRequest('tasks', 'POST', null, [body]);
    ...
    if (method === 'PUT' || method === 'PATCH') {
      ...
      const q = { id: `eq.${id}` };
      const r = await supabaseRequest('tasks', 'PATCH', q, body);
      return { statusCode: r.status, body: JSON.stringify(r.data), headers: cors() };
    }
    ...
    if (method === 'DELETE') {
      if (!id) return { statusCode: 400, body: 'id required', headers: cors() };
      const q = { id: `eq.${id}` };
      const r = await supabaseRequest('tasks', 'DELETE', q);
      return { statusCode: r.status, body: JSON.stringify(r.data), headers: cors() };
    }
