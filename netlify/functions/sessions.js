...
exports.handler = async function (event) {
  ...
  try {
    if (method === 'GET') {
      if (id) {
        const q = { id: `eq.${id}` };
        const r = await supabaseRequest('sessions', 'GET', q);
        return { statusCode: r.status, body: JSON.stringify(r.data), headers: cors() };
      }
      const r = await supabaseRequest('sessions', 'GET');
      return { statusCode: r.status, body: JSON.stringify(r.data), headers: cors() };
    }

    if (method === 'POST') {
      const body = JSON.parse(event.body || '{}');
      const r = await supabaseRequest('sessions', 'POST', null, [body]);
    ...
    if (method === 'PUT' || method === 'PATCH') {
      ...
      const q = { id: `eq.${id}` };
      const r = await supabaseRequest('sessions', 'PATCH', q, body);
      return { statusCode: r.status, body: JSON.stringify(r.data), headers: cors() };
    }
    ...
    if (method === 'DELETE') {
      if (!id) return { statusCode: 400, body: 'id required', headers: cors() };
      const q = { id: `eq.${id}` };
      const r = await supabaseRequest('sessions', 'DELETE', q);
      return { statusCode: r.status, body: JSON.stringify(r.data), headers: cors() };
    }
